---
bibliography: data/a3_references.bib
nocite: '@*'
---

# A Viral Metamorphosis: Novel Virus Pupavirus apiscarab Found in Honeybee Populations in China
written by: [Katarina Vucic](https://github.com/yourname *optional link*)
...
particularly detrimental virus is Chalkbrood disease, known to infect beehives and lead to the 
death of bee larvae. In this study, we address the challenge posed by viral infections, focusing 
...
narnavirus with a unique genomic and protein domain composition, representing a significant 
threat to bee populations. Contrary to existing assumptions about narnaviruses, our findings 
show that Pupavirus apiscarab exhibits a broad host range, infecting both Apis cerana and 
...
with this RdRp. The resulting search showed one long sense-strand ORF that had a length of 3099 
nucleotide bases. We then put this ORF (hereby termed ORF1) through NCBI Blast and found the
highest matches were to narnavirusus, and that they were found to have a percent identity of 
less than 37%, thus indicating that is virus is likely a highly diverged narnavirus (Figure 1).
```

![](data/img/ncbi_taxonomy.jpg)
Figure 1: NCBI BLAST Taxonomy of the P. apiscarab ORF1 (here termed ORF4). It is closely assosciated with other narnaviruses, but splits off from the rest early on in the tree. Figure generated by [NCBI Blast]. 

```
As a validation that P. apiscarab is a diverged narnavirus, we used InterProScan to see if
there were any other domains located in ORF1. Similar to narnaviruses, InterProScan predicted 
only one protein domain in this contig, that of the RdRp (Figure 2). Thus, we can conclude 
that P. apiscarab is a highly diverged narnavirus.
```
![](data/img/interproscan.png)
Figure 2: Predicted Protein Domains for ORF1 in P. apiscarab, generated by InterProScan [Paysan-Lafosse, 2022].
...
the lower scale of RdRp scores (Figure 3). To interrogate the RdRp a little further, we put 
the structure through AlphaFold2 and found that the prediction had low confidence, including 
in the 417-649 range where the RdRp is (Figure 4). This was further proof that the narnavirus
is highly diverged, and gave us the idea that it might be a unique virus.
```
![](data/img/palm_id.jpg)
Figure 3: Scan of P. apiscarab's RdRp palm sequence, indicating that the palmprint of the RdRp sequence is different that the typical RdRp. Figure generated with Serratus palmID [Babaian and Edgar, 2022].

![](data/img/seq_coverage.png)
Figure 4: AlphaFold2 Prediction using ColaFold for P. apiscarab shows poor sequencing coverage on regions containing the RdRp [AlphaFold2].
...
![](data/img/p_apiscarab_virus_figure.jpg)
Figure 5: The virus genome of Pupavirus apiscarab. ORF1's coverage is represented by the arrow, with specific regions coloured. The RdRp is the sequence between the C' and N' extension, and contains the A, B, and C motifs. The bottom scale shows the coverage profile of reads mapped to contig 1. Figure generated using SRA toolkit, bowtie2, and samtools, and inspired by [Gupta, 2023].
